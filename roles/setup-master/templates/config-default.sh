...
{% if groups['master'][0] in groups['minions'] %}
nodes=${nodes:-"{%- for host in groups['minions'] -%}{{ ansible_ssh_user }}@{{ hostvars[host]['ansible_all_ipv4_addresses'][-1] }} {% endfor %}"}
roles=${roles:-"a{% for host in groups['minions'] %}i {% endfor %}"}
{% else %}
nodes=${nodes:-"{{ ansible_ssh_user }}@{{ hostvars[groups['master'][0]]['ansible_all_ipv4_addresses'][-1] }}{% for host in groups['minions'] %} {{ ansible_ssh_user }}@{{ hostvars[host]['ansible_all_ipv4_addresses'][-1] }}{%- endfor -%}"}
roles=${roles:-"a{% for host in groups['minions'] %} i{%- endfor -%}"}
{% endif %}
...
NUM_MINIONS={{ groups['minions']|length }}
